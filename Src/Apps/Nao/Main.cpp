/**
 * @file Main.cpp
 * Implementation of the main() function for starting and stopping the module
 * framework.
 * @author Colin Graf
 */
...
SystemCall::Mode SystemCall::getMode() { return physicalRobot; }
...
static std::string getBodyId() {
    ...
    long bytesReceived =
        recv(socket, reinterpret_cast<char*>(receivedPacket), sizeof(receivedPacket), 0);
    if (bytesReceived >= 0)
        MsgPack::parse(
            receivedPacket, bytesReceived, [](const std::string&, const unsigned char*) {},
...
static void sitDown(bool ok) {
    static constexpr Joints::Joint numOfJoints =
        static_cast<Joints::Joint>(Joints::numOfJoints - 2);
    static const Angle targetAngles[numOfJoints] = {
        0_deg,  0_deg,  // Head

        51_deg, 3_deg,   15_deg,  -36_deg, -90_deg,  // Left arm
        0_deg,  0_deg,   -50_deg, 124_deg, -68_deg,
        0_deg,  // HipYawPitch and left leg
    ...
    if (!connect(socket, reinterpret_cast<sockaddr*>(&address), sizeof(address))) {
        ...
        // Determine current angles and whether sitting down is required, i.e. has
        // the hip stiffness?
        bool sitDownRequired = false;
        Angle startAngles[numOfJoints];
        std::array<float, 3> torsoAngles; /**< The addresses of torso angle data
                                             inside receivedPacket. */
        ...
        MsgPack::parse(
            ...
            [&sitDownRequired, &startAngles](const std::string& key, const unsigned char* p) {
                ...
                else if (category == "Stiffness" && (index == 8 || index == 13) &&
                         MsgPack::readFloat(p) > 0.f)
        ...
        // If sitting down is required, interpolate from start angles to target
        // angles.
        if (sitDownRequired) {
            float ratio = 0.f;
            while (ratio < 1.f) {
                ...
                const float phase =
                    0.5f * std::sin(ratio * Constants::pi - Constants::pi / 2.f) + 0.5f;
                ...
                MsgPack::parse(
                    ...
                    [&torsoAngles](const std::string& key, const unsigned char* p) {
                        ...
                        const std::string category = key.substr(0, pos);
                        const int index = std::stoi(key.substr(pos + 1));
                        if (category == "Angles") torsoAngles[index] = MsgPack::readFloat(p);
                    },
                    // Ignore ints and strings
                    [](const std::string&, const unsigned char*) {},
                    [](const std::string&, const unsigned char*, size_t) {});
                ...
                if (std::abs(torsoAngles[0]) > 45_deg || std::abs(torsoAngles[1]) > 45_deg) {
                    ...
                    for (int i = 0; i < numOfJoints; ++i) MsgPack::write(0.2f, p);
                } else
                    MsgPack::writeMapHeader(1, p);

                MsgPack::write("Position", p);
                MsgPack::writeArrayHeader(numOfJoints, p);
                // The shoulder pitch joints interpolate faster to avoid collisions of
                // the arms with the legs.
                const float shoulderPitchPhase = std::sqrt(std::min(1.f, ratio / 0.6f));
                for (int i = 0; i < numOfJoints; ++i) {
                    if (i == 2 || i == 18)
                        MsgPack::write(targetAngles[i] * shoulderPitchPhase +
                                           startAngles[i] * (1.f - shoulderPitchPhase),
                                       p);
                    else
                ...
                bytesReceived = recv(socket, reinterpret_cast<char*>(packet),
        ...
        MsgPack::write("Stiffness", p);
        MsgPack::writeArrayHeader(numOfJoints, p);
        for (int i = 0; i < numOfJoints; ++i) MsgPack::write(0.f, p);

        // Switch off all leds, except for the eyes (ok: blue, crashed: red)
        static std::string ledCategories[8] = {"LEye",  "REye",  "LEar",  "REar",
...
static std::string getBodyName(const std::string& bodyId) {
    ...
    if (!robotsStream.exists())
        ...
        fprintf(stderr,
                "Could not find bodyName in robots.cfg! BodyId: %s\nAssuming "
                "\"Default\".\n",
                bodyId.c_str());
        bodyName = "Default";
    }
...
int main(int argc, char* argv[]) {
    ...
    -w            use a watchdog for crash recovery and creating trace dumps\n",
        ...
        if (watchdog) {
            for (;;) {
                ...
                if (!normalExit) {
                    // Wait 100 ms before attempting to sit down. Otherwise, LoLA might
                    // send an invalid packet.
                    usleep(100000);

                    sitDown(false);

                    // Dump to file first, because writing to stderr may fail for various
                    // reasons.
#ifdef TARGET_ROBOT
        ...
        Settings settings(SystemCall::getHostName(), getBodyName(getBodyId()),
                          Constants::naoMotionCycleTime, Settings::nao);
        if (settings.playerNumber < 0 || settings.bodyName.empty()) return EXIT_FAILURE;
        ...
        // put the current body name in a file (at the moment only used in
        // downloadCalibration)
        ...
        // settings go out of scope here, but everything that needs it later makes a
        // copy.
    }
    ...
    bhumanStop();
    sitDown(true);
    if (shutdownNAO) static_cast<void>(!system("sudo systemctl poweroff &"));
