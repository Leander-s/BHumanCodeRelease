...
class Assert {
   public:
    ...
    /**
     * Dumps the content of the log message ring buffers to stderr or
     * /home/nao/bhdump.log
     * @param toStderr Whether to dump the content to stderr or
     * /home/nao/bhdump.log
...
/**
 * ASSERTs cond
 * if NDEBUG is defined the safe value will be returned in the case of a failing
 * conditional check it is recommended to use SAFE_ASSERT just for cond that are
 * fast evaluated
 */
#ifdef NDEBUG
 ...
 * This text is passed into a std::stringstream, thus FAIL("error " << 1) is a
 * valid expression.
 */
#ifdef NDEBUG
